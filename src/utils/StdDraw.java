...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	private static JMenuBar createMenuBar() {
		...
		JMenu options_tab = new JMenu("Options");
		menuBar.add(options_tab);
		JMenuItem save_graph_item = new JMenuItem("Save Graph");
		JMenuItem load_graph_item = new JMenuItem("Load Graph");
		save_graph_item.addActionListener(std);
		load_graph_item.addActionListener(std);
		options_tab.add(save_graph_item);
		options_tab.add(load_graph_item);
		JMenu algo_tab = new JMenu("Algorithms");
		menuBar.add(algo_tab);
		JMenuItem isc_item = new JMenuItem("isConnected");
		JMenuItem spd_item = new JMenuItem("shortestPathDist");
		JMenuItem sp_item = new JMenuItem("shortestPath");
		JMenuItem tsp_item = new JMenuItem("TSP");
		isc_item.addActionListener(std);
		spd_item.addActionListener(std);
		sp_item.addActionListener(std);
		tsp_item.addActionListener(std);
		algo_tab.add(isc_item);
		algo_tab.add(spd_item);
		algo_tab.add(sp_item);
		algo_tab.add(tsp_item);
		return menuBar;
	}
	...
	private double path(StringBuilder sb) {
		...
		if(gui.getGraphAlgo().shortestPath(src, dest)==null) 
			JOptionPane.showMessageDialog(null, "The isn't a path between those two nodes","Messege",0);

		else {
			ArrayList<node_data> lista = (ArrayList<node_data>) gui.getGraphAlgo().shortestPath(src, dest);
		...
		return (gui.getGraphAlgo().shortestPathDist(src, dest));
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "Save Graph":
			...
			if (fileName != null) {
				fileName = "saved graphs/" + fileName + ".txt";
				gui.getGraphAlgo().save(fileName);
			}
			break;
		...
		case "isConnected":
			...
			boolean b = (gui.getGraphAlgo().isConnected());
			if (b==true)
		...
		case "TSP":
			...
			StdDraw.setPenRadius(0.005);
			StdDraw.setPenColor(Color.green);
			ArrayList<node_data> tspList =  (ArrayList<node_data>) gui.getGraphAlgo().TSP(targets);
